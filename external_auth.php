...
function external_auth($user, $username, $password) {
  ...
  $auth_url = sprintf("%s?%s=%s&%s=%s", get_option('authentication_url'), get_option('login_param_name'),
                      $username, get_option('password_param_name'), $password);
  ...
  } else if($auth_response['result'] == 1) {
    $userobj = new WP_User();
